...
public class MinAndMaxWebTestGenerator {
	...
	String testInit(){
		return "\t@BeforeClass\n"
				...
				+ "\t\twd.get(\"file:///C:/depot/EE360T/src/pset6/minandmax.html\");\n"
				+ "\t}\n\n";
	}
			...
			for (String y : numTexts) {
				for (String z : numTexts) {
					for (String button : buttonTexts) {
						for (String radioButton : radioTexts) {
							sb.append("\t@Test\n");
							sb.append("\tpublic void test" + testIndex + "() {\n");
							sb.append("\t\tWebElement we = wd.findElement(By.id(\"" + radioButton + "\"));\n");
							sb.append("\t\twe.click();\n");
							...
							if(button.equals(buttonTexts[0])){
								if(x.equals(numTexts[0]) || y.equals(numTexts[0]) || z.equals(numTexts[0])) {
									...
									if(radioButton.equals(radioTexts[0])){
										...
										int max = Math.max(Math.max(Integer.parseInt(x), Integer.parseInt(y)), Math.max(Integer.parseInt(y), Integer.parseInt(z)));
										sb.append("\t\tassertEquals(\"max(" + x + ", " + y + ", " + z + ") = " + max + "\", output);\n");
									}
								}
							}
